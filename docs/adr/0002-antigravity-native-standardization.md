...
1. Standardize project-level agent configuration exclusively in `.agents/` (`rules/`, `skills/`, `agents/`, `hooks.json`, root `AGENTS.md`) without redundant legacy mirrors.
2. Implement `.antigravityignore` with aggressive token guards blocking lockfiles, heavy static assets, and test snapshots.
