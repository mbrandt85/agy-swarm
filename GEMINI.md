...
Welcome to `agy-swarm`, a portable multi-agent orchestration boilerplate and ruleset for Antigravity and Gemini CLI.
...
Specialized subagents are defined in `.agents/agents/` (and `.gemini/agents/`) using least-privilege toolsets and model tiering:
...
- Configured in `.agents/hooks.json` (and `.gemini/hooks.json`).
...
## Rules Index (`.agents/rules/` / `.gemini/rules/`)
All agents must adhere to repository rules:
...
4. **04-context-boundaries.md**: Respect `.antigravityignore` and `.geminiignore`; never inspect lockfiles unless instructed.
